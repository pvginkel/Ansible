...
- **Inventory contract** — `disk_resize` requires `vm_id` and `pve_node` on the target host. All managed VMs already carry these; new VMs added in inventory must too. The dev inventory mirrors `wrkscratch`'s `vm_id` / `pve_node` from prd because dev runs would otherwise miss them.
