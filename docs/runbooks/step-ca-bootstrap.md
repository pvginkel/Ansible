...
- **K8s API server certs** (prd + dev) carry a deliberately small
  SAN set:
  - `kubernetes-api[.home]` — prd HA VIP hostname. The VIP itself is
    set up in a follow-up slice; the SAN is in the policy now so the
    cert doesn't need re-issuing the day the VIP lands.
  - `kubernetes-api-dev[.home]` — dev alias pointing at wrkdevk8s.
  - `kubernetes`, `kubernetes.default`, `kubernetes.default.svc`,
    `kubernetes.default.svc.cluster.local` — the in-cluster names
    pods use to reach kube-apiserver.
  - `127.0.0.1` — same-node calls from kubelet / kube-proxy /
    controller-manager.
  - `172.17.0.1` — prd in-cluster `kubernetes` Service ClusterIP.
    First IP of prd's `--service-cluster-ip-range=172.17.0.0/16`.
  - `172.19.0.1` — dev (wrkdevk8s) equivalent. First IP of dev's
    `--service-cluster-ip-range=172.19.0.0/16`.
- Microk8s's stock self-signed cert advertises additional IP SANs
  (node LAN IP, workload-net IP, backplane IP, IPv6 equivalents,
  the unused upstream-default `10.152.183.1`). These are
  auto-generated by microk8s's `csr.conf.template` and nothing in
  this homelab relies on them — the replacement cert leaves them out
  on purpose.
- When the HA VIP slice lands, add the VIP IP to the `ip` list here
  and reload step-ca *before* re-issuing the k8s API server cert.
