...
| `/work/ArgoCDTools/aac-tools/image/homelab-root.crt` | Baked into the `aac-tools` image's trust store — the architecture generator and `arch-validate`, which Jenkins pulls and KubeCoder offers as a toolchain. It is how a run reaches `https://charts.home` for the chart dependency and `https://architecture.webathome.org` for the dataset and the validator. |
