...
- **Bring-up-tier hosts** (Ceph nodes, k8s nodes): cloud-init renders a static netplan from `vms.tf`'s per-NIC `addresses`/`gateway`/`nameservers` fields. `static_ip = true` on the `managed-vm` module suppresses the `homelab_dns_reservation` resource and tells the cloud-init template to write `/etc/netplan/50-cloud-init.yaml` directly. See "Ceph nodes / k8s nodes are static infrastructure" below for rationale.
