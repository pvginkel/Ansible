...
- **Ceph nodes are static infrastructure, not dynamic reservations**: `srvceph1/2/3` (and any future bring-up-tier host) opt out of the `homelab_dns_reservation` resource via `static_ip = true` on the `managed-vm` module. Their hostname → IP triples live exclusively in HelmCharts `configs/{prd,dev}/dnsmasq.yaml`'s static-hosts section, hand-curated alongside printers/IoT/network gear. Rationale: the dnsmasq sidecar runs in-cluster behind the registry; the registry container itself depends on Ceph storage to boot. Any chain that puts Ceph addressing behind the dynamic API creates a cold-boot ordering failure where Ceph can't be reached until the registry is up, but the registry can't start without Ceph. Static IPs sidestep the cycle entirely.
