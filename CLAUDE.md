...
- **Terraform lives in `terraform/`. State reads work from this pod; `plan`/`apply` do not.** Provider is `bpg/proxmox`; `terraform/{prd,scratch}/backend.tf` points at an http backend on `127.0.0.1:6061`. That is now served here too: `.kubecoder/config.yaml` runs the `terraform-backend-git` catalog service as a sidecar, so `terraform init` and state reads (`state list`, `show`) succeed via `cexec iac`. The backend URL names the git store (`pvginkel/TerraformState`, ref `main`), so this daemon and the one `iac-impl` starts on **srviac** resolve to the same state — reads here are the real thing, not a private copy. `terraform fmt` needs no state at all (`kc project lint` runs it).

  What still does not work is anything that contacts Proxmox: `terraform/prd` takes its credentials as variables (`proxmox_endpoint`, `proxmox_username`, `proxmox_password`, `dns_reservation_token`, `backup_server_token`), only `terraform.tfvars.example` is checked in, and the KubeCoder secret catalog carries none of them — so `plan`/`apply` fail here on missing variables. Those runs still happen in the `IaC/*` Jenkins pipelines, or by hand on srviac via `iac -c '…'`.
