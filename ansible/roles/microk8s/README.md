...
| `microk8s_kubelite_ready_timeout` | optional | `180` | Seconds the `Restart microk8s kubelite` handler waits for the node it just restarted to be ready again before failing. Also bounds what a wedged node costs the rest of a roll, since the handler holds its one-node-at-a-time slot for that long. |
...
- When it does fire, `Restart microk8s kubelite` carries its own one-node-at-a-time limit rather than borrowing the calling play's `serial:`: `throttle: 1` hands the restart to one host at a time, and the same task then waits for that node to be ready again. An apiserver node is ready once its own `https://127.0.0.1:16443/readyz` answers HTTP 200 — `/readyz` covers the datastore and informer sync that `/livez` leaves out. A worker runs no apiserver, and its 16443 is the apiserver proxy, which answers while kubelite is still down; so it reads its own Node through that proxy with `credentials/kubelet.config`, and is ready once the Node shows `Ready=True` with a `lastHeartbeatTime` later than the restart. Each probe gives up after 5 seconds (`curl -m 5`, `kubectl --request-timeout=5s`), and between probes the task checks `microk8s_kubelite_ready_timeout`; once that has elapsed it fails the host with `kubelite restarted, but got no <what it awaited> within <N>s (last seen: …)`. Restart and wait have to stay one task — Ansible runs a handler across every notified host before starting the next. The failure stops the roll only in a `serial: 1` play; in an un-serialised play the other notified hosts still restart. That is why `playbooks/renew-internal-tls.yml` renews the `k8s` group in its own last play under `serial: 1`, as `site-k8s.yml` applies under `serial: 1`.
- Under `--check` the restart handlers run no command, and `ansible.cfg`'s `display_skipped_hosts = False` hides even the skip. So `Restart microk8s`, `Rollout-restart coredns` and `Restart microk8s kubelite` each have a `debug` handler listening on their name. In check mode only, it reports the restart a real run would perform as changed on every notified host. Only a changed task notifies, so a converged node still reports `changed=0` — and a notifier that check mode itself skips notifies nothing: a due kube-apiserver leaf's issuance is skipped, so a `--check` run reports the pending re-issue but not the kubelite restart behind it. The Calico re-apply handler has no such report.
