...
- **Re-asserts `/etc/netplan/50-cloud-init.yaml`** when any `network_devices[*]` declares `addresses`. Mirrors the netplan that `terraform/prd/cloud-init.yaml.tftpl` writes on first boot, so a static-IP change in the host_var (which `vms.tf` also reads) lands on the running host without a rebuild. `-e baseline_netplan_apply=false` writes the render without applying it, for a change that moves the address Ansible is connected on; reboot afterwards (`docs/runbooks/static-address.md`). See `/work/AnsibleSpecs/decisions.md` "Cloud-init is a first-boot artefact".
- **Switches cloud-init's network stage off after first boot** (`/etc/cloud/cloud.cfg.d/99-homelab-network.cfg`, tag `cloud_init`). Proxmox's instance-id is a hash of the cloud-init data, so a changed `ipconfig0` makes the next boot a "new instance"; cloud-init would then re-render netplan from Proxmox's network-data, which describes net0 only, and bring the host up without its other NICs.
- **Pairs `.home` routing with public DNS.** When any `network_devices[*].nameservers` is defined (the same shape that triggers the static-netplan public-DNS render), the role drops `/etc/systemd/resolved.conf.d/home-routing.conf` with `DNS=10.2.1.2 10.2.1.3` and `Domains=home`. Per-link DNS (8.8.8.8/8.8.4.4) stays the default route; `*.home` and single-label names go to the in-cluster dnsmasq LBs. Tagged `netplan` together with the render. Hosts without explicit `nameservers` (DHCP-from-dnsmasq boxes like `wrkdev*`) get neither and resolve `.home` through the LAN dnsmasq natively. A handler restarts `systemd-resolved`, which briefly drops the local stub resolver. See `/work/AnsibleSpecs/slices/completed/home-dns-routing.md`.
