...
- **Re-asserts `/etc/netplan/50-cloud-init.yaml`** when any `network_devices[*]` declares `addresses`. Mirrors the netplan that `terraform/prd/cloud-init.yaml.tftpl` writes on first boot, so a static-IP change in the host_var (which `vms.tf` also reads) lands on the running host without a rebuild. `-e baseline_netplan_apply=false` writes the render without applying it, for a change that moves the address Ansible is connected on; reboot afterwards. `docs/runbooks/static-address.md` moves an address through cloud-init instead. See `/work/AnsibleSpecs/decisions.md` "Cloud-init is a first-boot artefact".
- **Does not switch cloud-init's network stage off**, although that would stop the re-render described in `docs/runbooks/static-address.md`. The same stage renames the primary NIC to `eth0` on every boot, keepalived and Calico on the k8s nodes are bound to that name, and with the stage disabled the NIC comes back as `enp6s18`.
