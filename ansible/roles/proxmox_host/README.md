...
- **Renewal** is threshold-gated by `internal_tls` (re-issue under 14 days left) and driven by the weekly `iac-scheduled-certs` job, which runs `playbooks/renew-internal-tls.yml` against every PVE node. `iac-scheduled-drift` is `--check`-only and checks at 7 days rather than 14, so it reports a renewal the certs job already missed, without ever signing one.
