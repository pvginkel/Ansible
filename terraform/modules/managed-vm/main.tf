...
#
# Skipped when var.static_ip = true (Ceph nodes, prd k8s nodes, srviac):
# those hosts are bring-up-tier infrastructure with hardcoded IPs in
# DnsmasqDeploy's static-hosts. See decisions.md "Ceph nodes and prd k8s
# nodes are static infrastructure".
